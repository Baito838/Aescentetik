...
function cms_wax($data){
    ...
    $query = "INSERT INTO products_wax VALUES ('','$nama_product', '$harga_product', '$foto_product', '$deskripsi')";
...
function upload_wax(){
    ...
    move_uploaded_file($tmp_file, 'upload_gambar/' . $nama_file_baru);
    return $nama_file_baru;
}

if (isset($_POST['cms_wax_edit'])){
    global $conn;
    ...
    if( $_FILES["foto_product"]["error"] === 4){
        ...
        $result = mysqli_query($conn, "SELECT foto_product FROM products_wax WHERE id = $id");
    ...
    $ubah = mysqli_query($conn, "UPDATE products_wax SET nama_product = '$nama_product', harga_product = '$harga_product' , foto_product = '$foto_product', deskripsi = '$deskripsi' WHERE id = '$_POST[id]'");
...
if (isset($_POST['hapus_wax'])){
    ...
    $hapus = mysqli_query($conn, "DELETE FROM products_wax WHERE id = '$_POST[id]'");
...
$product = query("SELECT * FROM products_wax");
?>
...
<body id="crud">
    <div class="table-bg">
        <div class="text-center text-light col" >
            <h1 class="title-product">Wax Sachet</h1>
        </div>
        <div class="button col d-flex justify-content-between mb-2">
            ...
            <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#tambah_product">
        ...
        <div class="tabel-overflow">
            <table class="table table-light table-hover table-responsive">
                ...
                <tbody class="table-warning">
                    ...
                    <td class="text-center"><div class="foto"><img class="foto_product" src="upload_gambar/<?= $row["foto_product"] ?>" alt=""><br><a href="upload_gambar/<?= $row["foto_product"] ?>" download></a></div></td>
                    <td class="text-center"><?= $row["deskripsi"] ?></td>
                    <td class="text-center justify-content-center flex-wrap gap-2">
                        <button type="button" class="btn btn-warning mb-2" data-bs-toggle="modal" data-bs-target="#edit<?= $no ?>">
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header edit">
                                    <h5 class="modal-title text-light" id="editLabel">Edit Produk Wax</h5>
                        ...
                        <div class="modal fade" id="hapus_product<?= $no ?>" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="hapus_productLabel" aria-hidden="true">
                            <div class="modal-dialog">
                                ...
                                <div class="modal-footer">
                                    ...
                                    <button class="btn btn-danger" type="submit" id="hapus_wax" name="hapus_wax">Hapus</button>
                                </div>
                                </div>
                            </div>
                        </div>
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header tambah">
              <h5 class="modal-title text-light" id="tambah_productLabel">Tambah Produk Wax</h5>
              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
            <form action="" method="post" enctype="multipart/form-data">
            ...
            <div class="modal-footer d-flex justify-content-end gap-2 flex-row">
                ...
                <button type="submit" class="btn btn-success" id="cms_wax" name="cms_wax">Tambahkan</button>
            </form>
            </div>
          </div>
        </div>
